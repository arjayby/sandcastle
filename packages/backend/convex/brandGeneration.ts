...
type DirectedGenerationContext = GenerationContext & {
...
function createLiveProvider(): BrandGenerationProvider {
  return {
    ...
    async createColor(ctx, context) {
      const result = await brandAgent.generateObject(
        ...
        { userId: context.ownerId },
        {
          ...
          prompt: regionPrompt("color", context),
        },
      );
    ...
    async createTypography(ctx, context) {
      ...
      return typographyGenerationSchema.parse(result.object);
    },
...
export const generate = internalAction({
  args: {
    projectId: v.id("brandProjects"),
    ownerId: v.string(),
    provider: v.union(v.literal("live"), v.literal("controlled")),
  },
  returns: v.null(),
  handler: async (ctx, { projectId, ownerId, provider: providerName }) => {
    ...
    const provider =
      providerName === "controlled" ? controlledProvider : createLiveProvider();
